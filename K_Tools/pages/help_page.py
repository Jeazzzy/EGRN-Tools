...
TOOLS = [
    ...
    },
    {
        ...
        "input": "Папка всего выпуска с вложенными папками pdf и xml либо одна из этих папок. Для XML задаётся допустимая точность в метрах.",
        "workflow": "Во вкладке PDF извлекается площадь без величины погрешности. Во вкладке XML программа читает XML внутри ZIP, сверяет индекс с папкой зоны, считает полигоны и точки без замыкающих повторов и проверяет delta_geopoint/geopoint_opred.",
        "result": "Отдельные таблицы результатов PDF и XML. Ошибки выделяются цветом и расшифровываются во всплывающей подсказке. Кнопка «Экспорт XLSX» сохраняет результаты на отдельные листы PDF и XML.",
    },
]
