...
TOOLS = [
    ...
    },
    {
        ...
        "workflow": "Во вкладке PDF извлекаются площадь и количество страниц. Во вкладке XML программа читает XML внутри ZIP, считает полигоны и точки без замыкающих повторов и проверяет delta_geopoint/geopoint_opred. Для ТЗ индекс сверяется с папкой зоны. Для НП проверяются папка населённого пункта, type_boundary и reg_numb_border. Режим смешанных планшетов разрешает совместную картометрическую погрешность 1 и 5 м. Для оглавления используется DOCX «Титульник для ОМГ» внутри выпуска либо выбранный пользователем шаблон.",
        "result": "Отдельные таблицы результатов PDF и XML. Ошибки выделяются цветом и расшифровываются во всплывающей подсказке. Результаты можно экспортировать в XLSX. Кнопка «Создать оглавление» формирует DOCX с названиями объектов из XML и вычисленными начальными страницами PDF.",
    },
]
